...
// Imports
import {ISwapRouter as IPancakeSwapRouter} from "@uniswap/v3-periphery/contracts/interfaces/ISwapRouter.sol";
import {IUniswapV3Pool} from "@uniswap/v3-core/contracts/interfaces/IUniswapV3Pool.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {ReentrancyGuard} from "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import {IV3SwapRouter as IUniswapBSCRouter} from "./lib/IV3SwapRouterBSCRouter.sol";
...
contract FlashLoanMain is ReentrancyGuard {
    ...
    IPancakeSwapRouter public immutable PANCAKESWAP_V3_ROUTER;
    IUniswapBSCRouter public immutable UNISWAP_V3_ROUTER;
    ...
    constructor(
        ...
        PANCAKESWAP_V3_ROUTER = IPancakeSwapRouter(_pancakeRouter);
        UNISWAP_V3_ROUTER = IUniswapBSCRouter(_uniswapRouter);
        token0 = IERC20(_token0);
        token1 = IERC20(_token1);
        pool = IUniswapV3Pool(
            PoolAddress.getPool(_v3Factory, _token0, _token1, _poolFee)
        );
    }
    ...
    function _place_swap_v3(
        ...
        // Approve the token for the specific router that will be used.
        address tokenIn = _params.tokenIn;

        if (_params.exchange == 0) {
            // PancakeSwap
            IERC20(tokenIn).approve(address(PANCAKESWAP_V3_ROUTER), _amountIn);

            IPancakeSwapRouter.ExactInputSingleParams
                memory swapParams = IPancakeSwapRouter.ExactInputSingleParams({
                    tokenIn: tokenIn,
            ...
            amountOut = PANCAKESWAP_V3_ROUTER.exactInputSingle(swapParams);
        } else if (_params.exchange == 1) {
            IERC20(tokenIn).approve(address(UNISWAP_V3_ROUTER), _amountIn);

            IUniswapBSCRouter.ExactInputSingleParams
                memory swapParams = IUniswapBSCRouter.ExactInputSingleParams({
                    tokenIn: tokenIn,
                    tokenOut: _params.tokenOut,
                    fee: _params.fee,
                    recipient: address(this),
